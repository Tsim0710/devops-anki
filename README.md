...
| Сборка | `build.py` |
| Шаблон веб-тренажёра | `web/drill.html` |
...
Повседневная поверхность: выбрал тему и уровень → вопрос → ответ → знаю/не знаю.
Никаких настроек, работает с телефона по ссылке.

`build.py` вшивает карты прямо в страницу (`build/drill.html`) — рендер тот же самый,
что уходит в Anki, так что расхождений между двумя поверхностями быть не может.
Прогресс (упрощённый Лейтнер: не знаю → в начало очереди, знаю → дальше) лежит
в localStorage браузера: он свой на каждом устройстве и Anki про него не знает.

Anki-экспорт остаётся для интервальных повторений: тренажёр гоняет вопросы,
Anki помнит, когда к ним вернуться.
